...
{
    public static function inspectRuntimeArchive(
        string $absolutePath,
        string $relativePath,
        ...
        $archive = RuntimeMetadata::detectRuntimeArchiveFormat($fileName);
        $installName = RuntimeMetadata::runtimeInstallDirectoryName($fileName);
        $inspection = $archive === 'zip'
            ? RuntimeZip::inspectZipRuntime($absolutePath, $branchPlatform)
            : RuntimeTar::inspectTarRuntime($absolutePath, $branchPlatform);

        $fileVersion = RuntimeMetadata::runtimeVersionFromArchiveName($installName);
        $metadataVersion = trim((string)($inspection['version'] ?? ''));
        $metadataVersionCore = RuntimeMetadata::runtimeVersionCore($metadataVersion);
        ...
        $versionCore = RuntimeMetadata::runtimeVersionCore($version);
        if ($versionCore === '') {
        ...
        $vendor = $inspection['vendor'] !== ''
            ? $inspection['vendor']
            : RuntimeMetadata::inferRuntimeVendor($relativePath);
        $distribution = $inspection['distribution'];
        $runtimeId = RuntimeMetadata::runtimeSlug(implode('-', array(
            $vendor,
            $distribution,
            $version,
            $platform,
        ...
        return array(
            ...
            'java_major' => RuntimeMetadata::runtimeMajorFromVersion($versionCore),
            'vendor' => $vendor,
            ...
            'stable' => RuntimeMetadata::runtimeIsStableVersion($version),
        );
    }
    public static function analyzeRuntimeArchiveEntries(
        array $entries,
        callable $readEntry,
        string $branchPlatform,
        string $inspection
        ...
        if (isset($filesByLowerName[$releaseKey])) {
            ...
            $release = RuntimeArchive::parseJavaReleaseFile($readEntry($releaseEntry['locator']));
        }
        ...
        $platform = $release !== array() ? RuntimePlatform::platformFromJavaRelease($release) : '';
    ...
    public static function normalizeRuntimeArchiveEntry(string $name): string
    {
    ...
    public static function parseJavaReleaseFile(string $content): array
    {
        $result = array();
