...
final class RuntimeRequest
{
    public static function parseRuntimeRequest(): array
    {
        ...
        if (preg_match('/^(?:(?:windows|linux)-(?:x86|x86_64|aarch64)|macos-(?:x86_64|aarch64))$/D', $platform) !== 1) {
            RuntimeSupport::fail(422, 'runtime_request_platform_invalid', 'The requested runtime platform is unsupported.', array(
                'received' => $platform,
            ));
        }
        ...
        if (preg_match('/^[0-9]+(?:\.[0-9]+)*$/D', $version) !== 1) {
            RuntimeSupport::fail(422, 'runtime_request_version_invalid', 'version must be a Java major such as 17 or an exact version such as 17.0.16.', array(
                'received' => $version,
            ));
        }
        ...
        if (!in_array($distribution, array('any', 'jdk', 'jre'), true)) {
            RuntimeSupport::fail(422, 'runtime_request_distribution_invalid', 'distribution must be any, jdk or jre.', array(
