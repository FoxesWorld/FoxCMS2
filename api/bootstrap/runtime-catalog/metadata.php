...
function runtimeVersionFromArchiveName(string $name): string
{
    ...
    foreach ($patterns as $pattern) {
        if (preg_match($pattern, $name, $match) === 1) {
            return runtimeNormalizeVersion((string)$match[1]);
        }
    }
    return '';
}
function runtimeNormalizeVersion(string $version): string
{
...
{
    $normalized = runtimeNormalizeVersion($version);
    if ($normalized === '') {
        return '';
    }
...
function runtimeVersionsMatchExact(string $candidate, string $requested): bool
{
    $candidate = runtimeNormalizeVersion($candidate);
    $requested = runtimeNormalizeVersion($requested);
    ...
    return hash_equals(strtolower(runtimeVersionCore($candidate)), strtolower(runtimeVersionCore($requested)));
}
function compareRuntimeVersions(string $left, string $right): int
{
    $leftNormalized = runtimeNormalizeVersion($left);
    $rightNormalized = runtimeNormalizeVersion($right);
    ...
    $leftStable = runtimeIsStableVersion($leftNormalized);
    $rightStable = runtimeIsStableVersion($rightNormalized);
    if ($leftStable !== $rightStable) {
        return $leftStable ? 1 : -1;
    }
    return strnatcasecmp($leftNormalized, $rightNormalized);
}
function runtimeBuildProfileSelector(int $javaMajor, array $versionsByPlatform): string
{
    if ($javaMajor < 1) {
    ...
    foreach ($versionsByPlatform as $platform => $version) {
        ...
        $version = runtimeNormalizeVersion((string)$version);
...
function runtimeParseProfileSelector(string $selector): ?array
{
    ...
    foreach ($segments as $segment) {
        ...
        $version = runtimeNormalizeVersion((string)$match[2]);
        if ($version === '' || runtimeMajorFromVersion($version) !== $major || isset($versions[$platform])) {
            return null;
        }
    ...
    return array(
        'java_major' => $major,
        'versions' => $versions,
        'selector' => runtimeBuildProfileSelector($major, $versions),
    );
}
